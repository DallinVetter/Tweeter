...
public class MainPresenterUnitTest {
    ...
    public void testPostStatus_postFailed() {
        ...
        postStatus();
        verifyMessage("Failed to get post status: something went wrong");
    }
    ...
    public void testPostStatus_postFailedWithException() {
        ...
        postStatus();
        verifyMessage("Failed to get post status because of exception: this exception");
    }

    private void runDoAnswerWhen(answerBase answer) {
        Mockito.doAnswer(answer).when(mockStatusService).postStatus(Mockito.any(), Mockito.any(), Mockito.anyString(),
                Mockito.anyString(), Mockito.anyList(), Mockito.anyList(), Mockito.any(SimpleNotificationObserver.class));
    }

    private abstract static class answerBase implements Answer<Void> {
        @Override
        public Void answer(InvocationOnMock invocation) throws Throwable {
            ...
            assertTrue(parseMentions.size() == 0);
    ...
    private static class successAnswer extends answerBase {
        @Override
    ...
    private static class failureAnswer extends answerBase {
        @Override
        protected void handleOperation(SimpleNotificationObserver observer) {
            observer.handleFailure("something went wrong");
        }
    }

    private static class exceptionAnswer extends answerBase {
        @Override
        protected void handleOperation(SimpleNotificationObserver observer) {
            observer.handleException(new Exception("this exception"));
        }
    }

    private void postStatus() {
        mainPresenterSpy.statusPost("This is a test string");
    }
